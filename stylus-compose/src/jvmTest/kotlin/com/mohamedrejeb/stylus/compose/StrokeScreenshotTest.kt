...
/**
 * Visual regression harness for the Skia `drawVertices` ink renderer.
 *
 * Renders a fixed corpus of stroke shapes through the production
 * `tessellateRibbon` → `drawTessellatedStroke` pipeline to PNG files
 * under `build/reports/stroke-screenshots/`. There's no automated
 * pixel comparison yet — the goal is to make geometry regressions
 * visible without dragging a real pen across a window. After a change
 * to the tessellator, run:
 *
 *     ./gradlew :stylus-compose:jvmTest
 *
 * and eyeball the PNGs. Anything obviously wrong (visible spikes,
 * missing caps, broken ribbons) shows up immediately.
 *
 * Coverage notes:
 *  - **Straight** strokes pin the steady-state body width.
 *  - **L-shape** isolates a single 90° join.
 *  - **Zigzag / spiral** stress sharp consecutive joins, which is
 *    where miter spikes and inner-edge overlap show up first.
 *  - **U-turn** is the cusp fallback path.
 *  - **S-curve** stresses smooth direction change with consistent width.
 *  - **Variable pressure** exercises width modulation along a single
 *    stroke, including the pressure-curve shaping.
 *
 * Each scenario runs at a thin (size 5) and a thick (size 40) brush
 * so the artefact families that only appear at large widths show up.
 */
class StrokeScreenshotTest {
    ...
    @Test
    fun `L-shape 90 degree corner - thin`() {
        renderToPng("l-shape-thin.png", 400, 400) {
            drawStroke(buildLShape(), brush(5f))
        }
    }
    ...
    @Test
    fun `zigzag - thin`() {
        renderToPng("zigzag-thin.png", 600, 400) {
            drawStroke(buildZigzag(), brush(5f))
        }
    }
    ...
    @Test
    fun `S-curve - thin`() {
        renderToPng("s-curve-thin.png", 600, 400) {
            drawStroke(buildSCurve(), brush(5f))
        }
    }
    ...
    @Test
    fun `U-turn cusp - thin`() {
        renderToPng("u-turn-thin.png", 400, 400) {
            drawStroke(buildUTurn(), brush(5f))
        }
    }
    ...
    private fun buildLShape(): List<PenStrokePoint> {
        // Walk right, then turn 90° and walk down.
    ...
    private fun buildZigzag(): List<PenStrokePoint> {
        // Five arms alternating up/down — each arm 80px wide, 100px tall.
    ...
    private fun buildUTurn(): List<PenStrokePoint> {
        // Walk right, then almost retrace back — half-turn ~178°.
        val out = (0..15).map { i -> PenStrokePoint(100f + i * 12f, 200f, 0.6f, i * 16L) }
        val back = (1..15).map { i ->
            // Slight vertical offset so it isn't a perfect overlap.
    ...
    private fun buildSpiral(): List<PenStrokePoint> {
        ...
        for (i in 0..120) {
            val t = i / 120f
            // Three turns, radius growing from 20 to 200.
    ...
    private fun buildVariablePressure(): List<PenStrokePoint> {
        // Pressure walks 0.1 → 1.0 → 0.1 along the stroke.
        val pts = mutableListOf<PenStrokePoint>()
        for (i in 0..60) {
            val t = i / 60f
            // Triangle wave on pressure: 0.1 up to 1.0 at midpoint, back down.
    ...
    private fun DrawScope.drawStroke(input: List<PenStrokePoint>, brush: PenBrush) {
        // Fill background white so anti-aliasing has the right backdrop.
        drawRect(Color.White, size = size)
        val smoothed = catmullRomSmooth(input)
    ...
    private val reportsDir: File by lazy {
        // Tests run with cwd at the module root. Point at build/reports
        // relative to that — survives Gradle's wipe-and-recreate cycles.
        File("build/reports/stroke-screenshots").apply { mkdirs() }
    }
}
